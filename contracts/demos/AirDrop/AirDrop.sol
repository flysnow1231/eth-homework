...
contract Airdrop {
    ...
    function multiTransferToken(
        ...
        console.log("multiTransferToken start. _token= ", _token);
        ...
        console.log("IERC20 token");
        uint _amountSum = getSum(_amounts); // 计算空投代币总量
        console.log("_amountSum=", _amountSum);
        // 检查：授权代币数量 >= 空投代币总量
       // console.log("msg.sender=, token.add=", msg.sender , address(this) ,token.allowance(msg.sender, msg.sender));

        // console.log("msg.sender=", token.allowance(msg.sender));
        ...
        for (uint256 i; i < _addresses.length; i++) {
            console.log(
                "tansfer start. sender=, _addresses[i]=, amount=",
                msg.sender,
                _addresses[i],
                _amounts[i]
            );
