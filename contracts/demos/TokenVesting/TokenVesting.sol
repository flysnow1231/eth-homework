...
contract TokenVesting {
    ...
    function release(address token) public {
        ...
        console.log("releaseable= ", releasable);
        // 更新已释放代币数量   
    ...
    function vestedAmount(address token, uint256 timestamp) public view returns (uint256) {
        ...
        if (timestamp < start) {
            ...
            console.log("timestamp > start + duration");
            return totalAllocation;
        } else {
            console.log("vested=", (totalAllocation * (timestamp - start)) / duration);

            return (totalAllocation * (timestamp - start)) / duration;
        }
    }

    function getReleased(address beneficiary) external view returns(uint){
        uint  released =   erc20Released[msg.sender];
        return released;
    }
}